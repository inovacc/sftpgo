...
`convertusers` is a very simple command line client, written in python, to import users from other stores. It requires
`python3` or `python2`.
...
Please note that when importing Linux/Unix users the input file is not required: `/etc/passwd` and `/etc/shadow` are
automatically parsed. `/etc/shadow` read permission is typically granted to the `root` user only, so you need to execute
`convertusers` as `root`.
